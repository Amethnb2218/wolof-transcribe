...
MAX QUALITÉ EN 1H — PARTIE 6 (samples 15000-18000)
"""

# CELLULE 1 :
# !pip uninstall -y triton triton-nightly
# !pip install -q triton==3.0.0 bitsandbytes==0.43.3 peft transformers==4.46.0 datasets==3.1.0 accelerate==1.1.0 evaluate jiwer soundfile librosa scipy
...
OUTPUT_DIR = "/content/whisper-wolof-max-p6"
...
print(f"\n{'='*60}")
print(f"   TRAINING WOLOF — MAX 1H — PARTIE 6")
print(f"   Samples 15000-18000")
...
# 1. DATASET
print("\nTéléchargement dataset (samples 15000-18000)...")
train_ds = load_dataset("soynade-research/Wolof-ASR-Data", split="train[15000:18000]")
...
print("Bonus: galsenai/wolof_tts (samples 10000-12000)...")
try:
    galsen = load_dataset("galsenai/wolof_tts", split="train[10000:12000]")
...
print(f"\n{'='*60}")
print(f"   ENTRAINEMENT MAX — PARTIE 6 (samples 15000-18000)")
...
FINAL_DIR = "/content/whisper-wolof-final-p6"
trainer.save_model(FINAL_DIR)
processor.save_pretrained(FINAL_DIR)
tokenizer.save_pretrained(FINAL_DIR)

print(f"\n{'='*60}")
print(f"   TERMINÉ — PARTIE 6 !")
print(f"   WER: {results['eval_wer']:.1f}%")
print(f"{'='*60}")

import shutil
shutil.make_archive("/content/whisper-wolof-model-p6", "zip", FINAL_DIR)
from google.colab import files
print("\n>>> Téléchargement du modèle PARTIE 6... <<<")
files.download("/content/whisper-wolof-model-p6.zip")
