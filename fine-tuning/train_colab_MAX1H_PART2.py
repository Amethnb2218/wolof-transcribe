...
MAX QUALITÉ EN 1H — PARTIE 2 (samples 3000-6000)
Données DIFFÉRENTES du compte 2 — à fusionner après
"""

# ============================================================
# CELLULE 1 : Installer d'abord avec !pip dans une cellule séparée :
# !pip install -q -U bitsandbytes==0.43.3 peft transformers==4.46.0 datasets==3.1.0 accelerate==1.1.0 evaluate jiwer soundfile librosa scipy
# ============================================================
...
# ============================================================
# CONFIG — IDENTIQUE AU COMPTE 2 MAIS DONNÉES DIFFÉRENTES
# ============================================================
BASE_MODEL = "openai/whisper-large-v3"
OUTPUT_DIR = "/content/whisper-wolof-max-p2"
...
print(f"\n{'='*60}")
print(f"   TRAINING WOLOF — MAX 1H — PARTIE 2")
print(f"   Samples 3000-6000 (différents du compte 2)")
print(f"   {MAX_STEPS} steps, LoRA étendu")
print(f"{'='*60}")
print(f"\nGPU: {torch.cuda.get_device_name(0)} ({torch.cuda.get_device_properties(0).total_memory / 1e9:.1f} GB)")

# ============================================================
# 1. DATASET — samples 3000 à 6000 (DIFFÉRENT du compte 2)
# ============================================================
print("\nTéléchargement dataset (samples 3000-6000)...")
train_ds = load_dataset(
    "soynade-research/Wolof-ASR-Data",
    split="train[3000:6000]",
)
...
# Bonus galsenai (partie différente)
print("Bonus: galsenai/wolof_tts (samples 2000-4000)...")
try:
    galsen = load_dataset("galsenai/wolof_tts", split="train[2000:4000]")
...
print(f"\n{'='*60}")
print(f"   ENTRAINEMENT MAX QUALITÉ — PARTIE 2")
print(f"   {MAX_STEPS} steps — LoRA r=64 étendu")
print(f"   Données: samples 3000-6000 (DIFFÉRENT du compte 2)")
...
# ============================================================
# 10. EVAL + SAVE + DOWNLOAD
# ============================================================
print("\n\nÉvaluation finale...")
results = trainer.evaluate()
print(f"\n   WER: {results['eval_wer']:.1f}%")

FINAL_DIR = "/content/whisper-wolof-final-p2"
trainer.save_model(FINAL_DIR)
processor.save_pretrained(FINAL_DIR)
tokenizer.save_pretrained(FINAL_DIR)

print(f"\n{'='*60}")
print(f"   TERMINÉ — PARTIE 2 !")
print(f"   WER: {results['eval_wer']:.1f}%")
print(f"{'='*60}")

import shutil
shutil.make_archive("/content/whisper-wolof-model-p2", "zip", FINAL_DIR)
from google.colab import files
print("\n>>> Téléchargement du modèle PARTIE 2... <<<")
files.download("/content/whisper-wolof-model-p2.zip")
