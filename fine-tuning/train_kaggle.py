...
Kaggle GPU T4 x2
Colle tout ce code dans une seule cellule Kaggle et lance.
"""
...
# ============================================================
import subprocess
subprocess.run("pip install -q transformers==4.46.0 datasets==3.1.0 accelerate==1.1.0".split())
subprocess.run("pip install -q evaluate jiwer soundfile librosa scipy".split())
...
OUTPUT_DIR = "/kaggle/working/whisper-wolof-mega"
BATCH_SIZE = 8
...
print(f"\n{'='*60}")
print(f"   ENTRAINEMENT ({torch.cuda.device_count()}x T4)")
...
print(f"{'='*60}")
print(f"\n   Le modele est dans Output > whisper-wolof-mega/")
print(f"   Telecharge-le depuis Kaggle et place-le dans:")
print(f"   wolof-transcriber/backend/models/whisper-wolof-mega/")
