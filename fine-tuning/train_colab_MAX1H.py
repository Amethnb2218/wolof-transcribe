...
MAX QUALITÉ EN 1H — Colab T4
Stratégie : peu de données, beaucoup d'epochs, LoRA étendu, LR agressif
"""
...
# ============================================================
# CONFIG — MAXIMUM QUALITÉ EN 1H
# ============================================================
BASE_MODEL = "openai/whisper-large-v3"
OUTPUT_DIR = "/content/whisper-wolof-max"

BATCH_SIZE = 2
EVAL_BATCH_SIZE = 4
GRADIENT_ACCUMULATION = 2   # Batch effectif = 4 → plus de gradient updates
LEARNING_RATE = 3e-4        # Agressif pour convergence rapide
WARMUP_STEPS = 20
MAX_STEPS = 750             # ~45 min de training pur
EVAL_STEPS = 750            # 1 seule éval à la fin
SAVE_STEPS = 750
LOGGING_STEPS = 10
AUGMENT_PROB = 0.3
...
print(f"\n{'='*60}")
print(f"   TRAINING WOLOF — QUALITÉ MAXIMUM EN 1H")
print(f"   {MAX_STEPS} steps, LoRA étendu, multi-epoch")
print(f"{'='*60}")
print(f"\nGPU: {torch.cuda.get_device_name(0)} ({torch.cuda.get_device_properties(0).total_memory / 1e9:.1f} GB)")

# ============================================================
# 1. DATASET — 3000 samples (~6h) → 2+ epochs en 750 steps
# Moins de données + plus de passes = meilleur apprentissage
# ============================================================
print("\nTéléchargement dataset (3000 samples = ~6h audio)...")
...
try:
    galsen = load_dataset("galsenai/wolof_tts", split="train[:2000]")
...
# ============================================================
# 7. MODELE — LoRA ÉTENDU pour max qualité
# ============================================================
...
# LoRA ÉTENDU — plus de modules ciblés = meilleure adaptation
lora_config = LoraConfig(
    r=64,                    # Rang élevé = plus de capacité
    lora_alpha=128,
